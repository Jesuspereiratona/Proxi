...
- [x] Dado un borrador sin `fecha_cierre`, cuando se intenta enviar a revisión, entonces responde 422 `OFERTA_SIN_FECHA_CIERRE`
- [x] Dada una fecha de cierre pasada, cuando se envía a revisión, entonces responde 422 `OFERTA_FECHA_CIERRE_INVALIDA`
- [x] Dada una empresa `pendiente`, cuando envía a revisión, entonces responde 422 `EMPRESA_NO_VALIDADA` (era 403 en el planteamiento original; `services/empresas/reglas.js` de Fase 2 ya la lanza como `ReglaDeNegocio`, igual que `EMPRESA_CIERRES_PENDIENTES` — se alinea la spec al código existente, no al revés)
- [x] Dada una empresa con un cierre sin declarar de hace más de 7 días, cuando envía a revisión, entonces responde 422 `EMPRESA_CIERRES_PENDIENTES`
- [x] Dada una oferta `en_revision`, cuando coordinación aprueba, entonces queda `publicada` con `fecha_publicacion` y hay un evento registrado
- [x] Dada una oferta `publicada`, cuando la empresa la cierra sin motivo, entonces responde 422
- [x] Dada una oferta `publicada`, cuando la empresa la cierra con motivo `contratado`, entonces queda `cerrada` con `resultado_declarado = true`
- [x] Dada una oferta `publicada` con fecha de cierre de ayer, cuando corre la tarea nocturna, entonces queda `cerrada`, `motivo_cierre = vencida`, `resultado_declarado = false`
- [x] Dada una oferta `cerrada`, cuando se intenta publicarla de nuevo, entonces responde 409 `OFERTA_TRANSICION_INVALIDA`
- [x] Dada una oferta `cerrada`, cuando un estudiante postula, entonces responde 422 `OFERTA_NO_VIGENTE` — **diferido a Fase 4**: no existe todavía el endpoint de postular; `services/ofertas/reglas.js verificarVigencia` ya está lista y probada por unidad para que Fase 4 la use
- [x] Dada una oferta de la empresa A, cuando la empresa B la consulta o edita, entonces responde 404
- [x] Dado el listado público, cuando se consulta, entonces solo devuelve ofertas `publicada` con fecha de cierre futura
- [x] Dada cualquier transición ejecutada, cuando se consulta `oferta_eventos`, entonces existe la fila con estado anterior, nuevo y actor
